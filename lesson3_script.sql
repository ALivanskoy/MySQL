...
SELECT * 
FROM staff 
ORDER BY salary DESC; -- порядок убывания

SELECT * 
FROM staff 
ORDER BY salary; -- порядок возрастания
...
SELECT post, SUM(salary) AS "total salary"
FROM staff
GROUP BY post;

-- 4. Найдите кол-во сотрудников с специальностью (post) «Рабочий» в возрасте от 24 до 49 лет включительно.

SELECT COUNT(*) AS "Number of young workers" 
FROM staff
WHERE post = "Рабочий" AND age BETWEEN 24 AND 49;

-- 5. Найдите количество специальностей

SELECT COUNT(DISTINCT post) AS "number of posts"
FROM staff;
